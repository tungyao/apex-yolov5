...
from apex_recoils.core import GameWindowsStatus, ReaSnowSelectGun, SelectGun
...
from apex_yolov5 import auxiliary, check_run
...
if __name__ == "__main__":
    ...
    MoverFactory.init_mover(
        mouse_model=global_config.mouse_model, mouse_mover_params=global_config.available_mouse_models
    )
    SelectGun.select_gun = SelectGun.SelectGun(
        ...
        image_comparator=NetImageComparator(LogFactory.logger(), global_config.image_base_path),
    ...
    if global_config.rea_snow_gun_config_name != "" or global_config.joy_move:
        rea_snow_select_gun = ReaSnowSelectGun.ReaSnowSelectGun(
            logger=LogFactory.logger(), config_name=global_config.rea_snow_gun_config_name
        )
        ...
        # 压枪
        recoils_listener = RecoilsListener(
            logger=LogFactory.logger(),
    ...
    if global_config.mouse_model != "km_box_net":
        listener = pynput.mouse.Listener(on_click=apex_mouse_listener.on_click, on_move=apex_mouse_listener.on_move)
        listener.start()
