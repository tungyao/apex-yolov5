...
from apex_yolov5.Counter import reset_counter, sure_no_aim
...
class Config:
    ...
    def init(self):
        ...
        self.is_show_debug_window = self.get_config(
            self.config_data, "is_show_debug_window"
        )  # 可修改为True，会出现调试窗口
        ...
        self.available_mouse_models = self.get_config(
            self.config_data,
            "available_mouse_models",
            {
                "win32api": {},
                "km_box": {"VID/PID": "66882021"},
                "wu_ya": {"VID/PID": "046DC539"},
                "km_box_net": {"ip": "192.168.2.188", "port": "35368", "uuid": "8A6E5C53"},
                "fei_yi_lai": {"VID/PID": "C2160102"},
                "fei_yi_lai_single": {"VID/PID": "C2160301"},
                "logitech": {},
                "pan_ni": {"VID/PID": "1C1FC18A"},
            },
        )

        self.available_mouse_smoothing = self.get_config(
            self.config_data, "available_mouse_smoothing", ["win32api", "wu_ya"]
        )
        ...
        self.move_path_ny = self.get_config(
            self.config_data, "move_path_ny", self.move_path_nx
        )  # 锁定模式下鼠标移动速度
        ...
        # 移动路径倍率
        self.aim_move_path_nx = self.get_config(
            self.config_data, "aim_move_path_nx", self.move_path_nx
        )  # 锁定模式下鼠标移动速度
        self.aim_move_path_ny = self.get_config(
            self.config_data, "aim_move_path_ny", self.move_path_ny
        )  # 锁定模式下鼠标移动速度

        self.mouse_move_frequency = self.get_config(
            self.config_data, "mouse_move_frequency", 0.001
        )  # 锁定模式下鼠标移动速度
        self.mouse_move_frequency_switch = self.get_config(self.config_data, "mouse_move_frequency_switch", False)
        ...
        self.available_models = self.get_config(
            self.config_data,
            "available_models",
            {
                "apex标准": {"weights": "./apex_model/best2.engine", "data": "./apex_model/data2.yaml"},
                "apex区分敌我": {"weights": "./apex_model/best.engine", "data": "./apex_model/data.yaml"},
            },
        )
        ...
        self.aim_mouse_moving_radius = self.get_config(
            self.config_data, "aim_mouse_moving_radius", self.mouse_moving_radius
        )
        ...
        self.multi_stage_aiming_speed_toggle = self.get_config(
            self.config_data, "multi_stage_aiming_speed_toggle", False
        )
        self.based_on_character_box = self.get_config(self.config_data, "based_on_character_box", False)
        ...
        self.dynamic_screenshot_collection_window = self.get_config(
            self.config_data, "dynamic_screenshot_collection_window", 20
        )
        self.dynamic_screenshot_reduce_threshold = self.get_config(
            self.config_data, "dynamic_screenshot_reduce_threshold", 0.4
        )

        self.dynamic_screenshot_increase_threshold = self.get_config(
            self.config_data, "dynamic_screenshot_increase_threshold", 0.6
        )

        self.dynamic_screenshot_reduce_threshold_y = self.get_config(
            self.config_data, "dynamic_screenshot_reduce_threshold_y", 0.2
        )

        self.dynamic_screenshot_increase_threshold_y = self.get_config(
            self.config_data, "dynamic_screenshot_increase_threshold_y", 0.7
        )
        ...
        self.has_turbocharger = self.get_config(self.config_data, "has_turbocharger", ["专注", "哈沃克"])
        ...
        self.distributed_param = self.get_config(
            self.config_data, "distributed_param", {"ip": "127.0.0.1", "port": 12345}
        )
        ...
        if self.shot_width == 0 and self.shot_height == 0:
            ...
            self.half_shot_width, self.half_shot_height = (
                self.offset_shot_screen_x * 16,
                self.offset_shot_screen_y * 9,
            )
            self.shot_width, self.shot_height = (2 * self.half_shot_width, 2 * self.half_shot_height)
        else:
        ...
        self.auto_save_monitor = {
            "top": self.screen_center_y - 320,
            "left": self.screen_center_x - 320,
            "width": 640,
            "height": 640,
        }
    ...
    def change_shot_xy(self):
        ...
        #     self.reduce_shot_xy(self.dynamic_screenshot_step)
            ...
            sign_shot_x > self.dynamic_screenshot_increase_threshold
            or sign_shot_y > self.dynamic_screenshot_increase_threshold_y
        ):
            self.increase_shot_xy(self.dynamic_screenshot_step)
        elif (
            sign_shot_x < self.dynamic_screenshot_reduce_threshold
    ...
    def update_shot_other_data(self):
        self.left_top_x, self.left_top_y = (
            self.screen_center_x - self.half_shot_width,
