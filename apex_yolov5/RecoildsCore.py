...
from apex_yolov5.auxiliary import get_intention, set_intention
...
class RecoilsListener:
    ...
    def __init__(self, logger: Logger, mouse_listener: MouseListener, select_gun: SelectGun, config: Config):
        self.logger = logger
    ...
    def run(self):
        ...
        go_on_num = 0
        while True:
            if not self.config.recoils_toggle or not MoverFactory.mouse_mover().is_caps_locked():
                time.sleep(1)
                continue
    ...
    def move_index_xy(self, spec, current_index, point):
        ...
        index = (
            len(time_points) - 1
            if point > time_points[-1]
            else next((i - 1 for i, time_point in enumerate(time_points) if time_point > point), -1)
        )
        if index is not None and index >= 0 and current_index <= index:
            if len(x_values) >= current_index + 1:
                ...
                self.logger.print_log(f"缺失第[{current_index + 1}个轨迹，时间为{time_points[current_index]}])")
            return True
        return False
